...
|---|---|---|
| `map-idle` | Чистая загрузка без выбора | Имя и роль читаются как тихое авторство; consent скрыт; карта, оси, largest Garage node и `МАРШРУТ / 60 СЕК` не конфликтуют |

...
| `search-keyboard` | Открыть список на desktop и при `390×430`; нажать видимый `×`, `Enter`, `ArrowUp`, `Escape` | Пока список открыт, правая стрелка становится явным действием `Закрыть поиск`; `×` закрывает только список и сохраняет запрос, `Enter` открывает выбранный результат; на узком экране список сразу встаёт над строкой без въездного смещения, скроллится список, не страница; dock/nav скрываются и возвращаются |

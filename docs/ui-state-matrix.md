...
|---|---|---|
| `map-idle` | Чистая загрузка без выбора | Имя и роль читаются как тихое авторство на desktop; consent скрыт; карта и оси не конфликтуют; `МАРШРУТ / 60 СЕК` повторяет границы `.map-node-label` |
...
Кандидат hotfix скрывает авторство на ширинах `≤680px`, возвращает исходное
кадрирование карты, объединяет геометрию origin/map-label и стабилизирует
раскрытие мобильного поиска. Полные Chromium UI-контракты и WebKit-матрица
прошли локально. Это не production-PASS до публикации и контрольного
публичного рендера; физический Safari отдельно не проверялся. Датированные
доказательства записаны в

...
| `filter-*` | Переключить четыре категории и агрегат `ВСЁ` | `aria-pressed` каждой категории совпадает с фактической видимостью; `ВСЁ` активно визуально и семантически только когда включены все четыре категории, выключается при любом частичном составе и возвращает полный; последний видимый тип нельзя выключить |
...
| `console-view` | Группы `СОСТАВ` и `КООРДИНАТЫ`, полный/частичный состав и хронология | Колонка маркеров, baseline, разделитель, focus и соответствие видимости состояниям; иконка, подпись, active-индикатор и `aria-pressed` не расходятся |

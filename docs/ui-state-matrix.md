...
| `filter-*` | Переключить четыре категории и агрегат `ВСЁ`, оставив курсор над нажатой строкой | `aria-pressed` каждой категории совпадает с фактической видимостью; active использует signal-символ и постоянную рейку, hover не имитирует их и поэтому выключение читается сразу под неподвижным курсором; доступное имя категории стабильно; `ВСЁ` активно визуально и семантически только когда включены все четыре категории, выключается при любом частичном составе и возвращает полный; последний видимый тип нельзя выключить |

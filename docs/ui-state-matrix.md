...
| `settings-mobile-height` | Открыть общие настройки и отдельную приватность на mobile `390×844`, `393×650` и `320×568` | На высоком экране каждая оболочка обнимает свой фактический контент и оставляет после последнего действия только штатный внутренний отступ, а не свободную высоту viewport; header, intro, строки настроек, состояние, privacy-пояснения и действия разделены самостоятельными интервалами, строки имеют не менее `72 px`, controls — не менее `44 px`; на `320×568` обе альтернативы первого privacy-решения видимы в одной строке, а в общих настройках ограничивается оболочка и прокручивается только body; page overflow не возникает |

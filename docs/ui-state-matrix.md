...
кадрирование карты, связывает геометрию origin/axis-label и стабилизирует
раскрытие мобильного поиска. Chromium UI-контракты и WebKit-матрица прошли;
общий release-gate остаётся частью публикации. Это не production-PASS до
публикации и контрольного публичного рендера; физический Safari отдельно не
проверялся. Датированные доказательства записаны в

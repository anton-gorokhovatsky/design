...
The portfolio has no build step. Six deferred classic scripts execute in the
...
2. `signal-field.js` — decorative canvas and depth grid.
3. `map-data.js` — deterministic portfolio data.
4. `map-engine.js` — map geometry, relations, filters, and observation route.
5. `panels.js` — movable consoles, content panels, search, and URL state.
6. `favicon.js` — dynamic favicon and visibility lifecycle.
